...
public class OAuthCallbackFilter extends AuthenticatingFilter implements SessionRegenerationCapable {
    ...
    protected AuthenticationToken createToken(ServletRequest request, ServletResponse response) throws Exception {
        Tokens tokens = requestTokens(new AuthorizationCodeGrant(
                parseAuthorizationCode(WebUtils.toHttp(request)),
        ...
        if (tokens instanceof OIDCTokens) {
            return new OAuthAuthenticationToken(((OIDCTokens) tokens).getIDToken(), accessToken);
        } else {
    ...
    private AuthorizationCode parseAuthorizationCode(HttpServletRequest request) {
        ...
        try {
            authorizationResponse = AuthorizationResponse.parse(
        ...
        if (authorizationResponse.indicatesSuccess()) {
            ...
            State storedState = checkNotNull((State) SecurityUtils.getSubject()
                    .getSession()
                    .getAttribute(OAuthAuthenticationFilter.STATE_KEY), "No OAuth state found in security session");
